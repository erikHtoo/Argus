...
function App(){
 const [state,setState]=useState(null),[date,setDate]=useState(localDate()),[selected,setSelected]=useState(null),[settings,showSettings]=useState(false),[error,setError]=useState(''),[busy,setBusy]=useState(false);
 ...
 function step(n){const d=new Date(date+'T12:00:00');d.setDate(d.getDate()+n);setDate(localDate(d));setSelected(null);}
 ...
 const detail=sessions.find(x=>x.id===selected);
 return <main>
 ...
 <div className="sessions-overview" aria-label="Daily sessions">{sessions.map(x=><button key={x.id} className={'session-card '+x.winner.category+(selected===x.id?' chosen':'')} aria-expanded={selected===x.id} onClick={()=>setSelected(selected===x.id?null:x.id)}><span className="session-time">{clock(x.start)} – {clock(x.end)}</span><strong>{x.winner.label==='Idle'?'Idle':x.winner.label+' session'}</strong><span className="session-duration">{minutes((new Date(x.end)-new Date(x.start))/1000)}</span></button>)}</div>
 {!sessions.length&&<div className="empty"><h2>No sessions yet</h2><p>Your day takes shape after five minutes of activity.</p></div>}
 {detail&&<section className="hour-detail" aria-label="Session details"><div className="detail-heading"><div><p className="eyebrow">{clock(detail.start)} – {clock(detail.end)}</p><h2>{detail.winner.label==='Idle'?'Idle':detail.winner.label+' session'}</h2></div><button aria-label="Close session details" onClick={()=>setSelected(null)}>Close</button></div>
