...
import {summarizeActivity} from '../shared/activity.mjs';
...
export async function answerQuestion(state, question, apiKey = '') {
  ...
  if(state.settings.aiMode==='offline') {
    const activity=activityAnswer(state,question);if(activity)return activity;
  ...
  const content=JSON.stringify({today:localDate(),question,tasks:priorities,activityToday:summarizeActivity(state.sessions,localDate()),goals:state.goals.map(({title,target})=>({title,target})),evidence:sources.map((s,i)=>({citation:i+1,time:s.at,text:s.text.slice(0,1800)}))});
